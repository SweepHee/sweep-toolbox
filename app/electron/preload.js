...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  noteSetCollapsed: (noteId, collapsed) =>
    ipcRenderer.invoke("note-set-collapsed", noteId, collapsed),
  ...
  noteSetAlwaysOnTop: (noteId, value) =>
    ipcRenderer.invoke("note-set-always-on-top", noteId, value),
  noteSetOpacity: (noteId, value) =>
    ipcRenderer.invoke("note-set-opacity", noteId, value),
  noteSetReminder: (noteId, reminderAt) =>
    ipcRenderer.invoke("note-set-reminder", noteId, reminderAt),
  notePickImage: () => ipcRenderer.invoke("note-pick-image"),
  showNoteContextMenu: (state) =>
    ipcRenderer.invoke("show-note-context-menu", state),
  onNoteContextMenuAction: (cb) => {
    const handler = (_, action) => cb(action);
    ipcRenderer.on("note-context-menu-action", handler);
    return () =>
      ipcRenderer.removeListener("note-context-menu-action", handler);
  },
  onNotesUpdated: (cb) => {
    const handler = () => cb();
